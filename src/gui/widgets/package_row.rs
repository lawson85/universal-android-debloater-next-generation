...
pub fn handle_package_icon(
    ...
    // Open APK
    let file = File::open(&local_apk_path).map_err(|e| format!("Failed to open APK: {:?}", e))?;
    ...
    if let Some(png_name) = launcher_png {
        let mut png_file = archive.by_name(&png_name)
...
pub fn update(&mut self, message: &Message) -> Command<Message> {
    ...
    Command::perform(
        async move {
            ...
            match PackageRow::handle_package_icon(
                &package_name,
                &apks_dir,
                &icons_dir,
            ) {
...
let icon_path = self.icon_path.clone().unwrap_or_else(|| PathBuf::from("resources/Images/dummy.png"));
